...
| Envelope published but Praxis workflow does not advance, queue at zero, no DLQ activity | Result envelope's `businessKey` does not match the request's `businessKey`. Praxis's Receive Task correlates on businessKey; a non-matching value is silently dropped. Common cause: an adapter synthesizes its own businessKey (e.g. `<period>::<planillaName>`) for CLI runs and forgets to pass through the inbound request's value when broker-driven | `AbstractCaptureAdapter` / `AbstractSubmitAdapter` already prefer `params.businessKey` (set by the listener from the inbound envelope) over `outcome.businessKey()`. If you're hitting this, check that the listener is binding `params.businessKey` and that no adapter is overriding the envelope after `emitCaptureEnvelope()` runs |
| `IllegalStateException: Unresolved portal binding: params.<custom-name>` | Descriptor or adapter references a binding the listener doesn't emit (e.g. `params.fechaInicio`) | Use the standard listener-supplied bindings — see [PortalOnboarding.md "Descriptor design pitfalls"](PortalOnboarding.md#descriptor-design-pitfalls). The listener emits `params.from`, `params.to`, `params.planillaId`, `params.planillaName`, `params.firmId`, `params.businessKey`, `params.issuerRunId`, `params.sourcePortal`. Don't invent new names |
| `select` step times out with `did not find some options` after retries | Descriptor selects by visible label (`match: label`, the default) but the operator renamed the option in the portal UI | Switch the step to `match: value` and bind the underlying `<option value="...">` (typically a stable id); confirm the value attribute is what the portal renders |
| Run fails post-login with `waitForSelector` timeout on a nav element, only when `sessionReused=true` | Portal stores auth in `sessionStorage` or in-memory JWT, not cookies/localStorage. Playwright's `storageState()` captures nothing useful, so reuse lands logged-out | Set `session.ttlMinutes: 0` in the descriptor to force fresh login every run; delete `~/.financeagent/sessions/<portalId>.enc` to clear the stale state. Don't enable session reuse without verifying empirically that a saved session round-trips |
