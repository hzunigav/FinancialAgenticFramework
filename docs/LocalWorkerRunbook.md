...
| Praxis workflow stuck on Wait, **worker log shows** `cannot publish error result — routing to DLQ` followed by `SchemaValidationException` | The worker threw mid-run, tried to publish a FAILED/DUPLICATE/minimal envelope, and that error envelope itself failed schema validation → message DLQ'd, Praxis never heard back. The error-envelope path is **also** validate-on-publish. | The schema violation in the error envelope is the **secondary** bug. Read backward in the worker log past the DLQ trace to find the **original** exception that triggered the error path (credential lookup miss, descriptor binding error, portal navigation failure, etc.). Fix the underlying cause first; if the error envelope shape is itself wrong, mirror the listener's error-builder in `SchemaValidatorPojoTest` per [feedback_pojo_validation_test_required](../../../.claude/projects/e--Work-2026-FinanceAgentFramework/memory/feedback_pojo_validation_test_required.md). |
| `IllegalStateException: No credentials found for portal '<id>' client '<dashed-cedula>'` | Praxis emits `clientIdentifier` with cédula-jurídica dashes (e.g. `3-101-680139`); old worker builds did exact-string lookup against the dash-free key. | Rebuild against worker ≥ 2026-04-29 PM — `CredentialsProvider.normalizeClientId` strips dashes + whitespace before lookup; either wire form resolves to the same secret. |
