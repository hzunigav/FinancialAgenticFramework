...
- **Org switch — by SHORT-CODE, not tenant UUID (contract impact, DONE).** The switcher uses `OrganisationLogin/!<shortcode>` links (Demo Company = `!0X0!!`); org URL is `go.xero.com/app/!<shortcode>/...`. The tenant UUID is never a UI selector. → **Contract changed:** `xeroShortCode` is now **required** on `BankStatementTask` (the deterministic UI selector; agent deep-links by it, falls back to `xeroOrgName` in the switcher), and **`xeroOrgUuid` relaxed to optional** (API cross-check only — not UI-obtainable, so not forced on UI-only orgs). `xeroOrgName` stays as fallback selector + label. Supersedes the S0.2 "switch by UUID" assumption. Praxis captures the short-code per org (it's in the org URL).
